...
class LossOptimizerHandlerAccumulateSamples(LossOptimizerHandlerBase):
    """
        loss and optimizer handler to invoke loss.backward() every batch, 
        but invoke optimizer.step() and optimizer.zero_grad() only after the defined num of samples were accumulated

        Args:
            apply_on_phase - see in CallbackBase
            apply_on_states - see in CallbackBase
            min_num_samples_before_backprop - the amount of samples to wait before invoking optimizer.step() and optimizer.zero_grad()
    ...
    def __init__(self, 
                    ...
                    min_num_samples_before_backprop:int=1,
                    verbose:int=0):
        super(LossOptimizerHandlerAccumulateSamples, self).__init__(apply_on_phase, apply_on_states)
        self.min_num_samples_before_backprop = min_num_samples_before_backprop
        self.verbose = verbose
        self._sample_count = 0

    def _should_backprop(self, callback_context: CallbackContext):
        if callback_context.sample_count - self._sample_count >= self.min_num_samples_before_backprop:
            self._sample_count = callback_context.sample_count
            return True
    ...
    def __call__(self, callback_context: CallbackContext):
        ...
        if self._should_backprop(c):
            if self.verbose:
                print(f'[LossOptimizerHandlerAccumulateSamples] - collected {self.min_num_samples_before_backprop} samples, calling optimizer step and zero_grad')
