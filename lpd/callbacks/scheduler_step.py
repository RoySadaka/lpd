...
class SchedulerStep(CallbackBase):
    ...
    def __call__(self, callback_context: CallbackContext):
        if self.verbose and callback_context.trainer.scheduler is None:
            print('[SchedulerStep] - no scheduler defined in trainer')
            return

        current_lrs = get_lrs_from_optimizer(callback_context.trainer.optimizer)
        ...
        new_lrs = get_lrs_from_optimizer(callback_context.trainer.optimizer)
